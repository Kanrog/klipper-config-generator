// ============================================
// KLIPPER CONFIG GENERATOR - app.js
// ============================================
...
const SECONDARY_MCU_PRESETS = {
    ...
    'ebb36-v1.1': {
        ...
        type: 'toolhead',
        connectionType: 'canbus',
        description: 'BigTreeTech EBB36 CAN toolhead board v1.1',
    ...
    'sht36': {
        ...
        type: 'toolhead',
        connectionType: 'canbus',
        description: 'Mellow Fly SHT36/42 CAN toolhead board',
        configFile: null
    },
    'duet-1lc': {
        name: 'Duet3 1LC',
        ...
        type: 'toolhead',
        connectionType: 'canbus',
        description: 'Duet3 1LC CAN toolhead board',
        configFile: null
    },
    'rpi': {
        name: 'Raspberry Pi (Host MCU)',
        mcuName: 'rpi',
        type: 'host',
        connectionType: 'linux',
        description: 'Linux host MCU for ADXL, GPIO, etc.',
        configFile: null
    },
    'mmu': {
        name: 'MMU/ERCF Board',
        ...
        type: 'mmu',
        connectionType: 'usb',
        description: 'Multi-material unit controller',
        configFile: null
    },
    'expansion': {
        ...
        type: 'expansion',
        connectionType: 'usb',
        description: 'Additional stepper drivers',
        configFile: null
    }
};
    ...
    modal.innerHTML = `
        <div class="mcu-modal">
            <div class="mcu-modal-header">
                <h3 class="mcu-modal-title">🔧 Add Expansion Board</h3>
                ...
                <div class="mcu-item-body">
                    ...
                    <div class="mcu-field">
                        ...
                        <select onchange="updateSecondaryMcuConnectionType('${mcu.id}', this.value)">
                            ...
                            <option value="linux" ${mcu.connectionType === 'linux' ? 'selected' : ''}>Linux Host</option>
                        </select>
                    </div>
...
function countStepperDrivers(configText) {
    ...
    const boardPatterns = [
        ...
        { pattern: /skr.*pico/i, drivers: 4 },
        { pattern: /skr.*3/i, drivers: 5 },
        { pattern: /manta.*m8p/i, drivers: 8 },
        { pattern: /manta.*m5p/i, drivers: 5 },
        { pattern: /manta.*m4p/i, drivers: 4 },
        { pattern: /octopus/i, drivers: 8 },
        { pattern: /spider/i, drivers: 8 },
    ];
    
    for (const bp of boardPatterns) {
        if (bp.pattern.test(configText)) {
            return bp.drivers;
        }
    }
...
function updateKinematicsOptions() {
    const selected = document.getElementById('kinematicsSelect').value;
    ...
    if (kinematics.usesDelta) {
        ...
        zMotorsGroup.style.display = 'grid';
    }
...
function updateDriverWarning() {
    const driverCount = window.currentConfigData.driverCount || 0;
    const zMotorCount = parseInt(document.getElementById('zMotorCount').value) || 1;
    const kinematicsKey = document.getElementById('kinematicsSelect').value;
    const kinematics = KINEMATICS[kinematicsKey];
    
    let neededDrivers = 3 + 1;
    if (!kinematics.usesDelta) {
        neededDrivers = 2 + zMotorCount + 1;
    }
    
    let warningEl = document.getElementById('driverWarning');
    
    if (driverCount > 0 && neededDrivers > driverCount) {
        if (!warningEl) {
        ...
        warningEl.innerHTML = `
            ...
            <div class="warning-content">
                <strong>Driver Count Warning</strong>
                <p>Your configuration needs ${neededDrivers} drivers but this board only has ${driverCount}. Consider adding a secondary MCU for additional drivers.</p>
            </div>
        `;
    } else if (warningEl) {
        warningEl.remove();
    ...
    if (!sensorlessCheckbox.checked && (!defaults.hasPhysicalEndstopX || !defaults.hasPhysicalEndstopY)) {
        ...
        warningEl.innerHTML = `
            ...
            <div class="warning-content">
                ...
                <p>If you disable sensorless homing, you'll need to manually add <code>endstop_pin</code> definitions.</p>
            </div>
        `;
...
function extractDefaultValues(configText, sections) {
    ...
    if (isDelta) {
        ...
        if (stepperA) {
            const radiusMatch = stepperA.content.match(/arm_length:\s*([\d.]+)/);
            if (radiusMatch) defaults.deltaArmLength = parseFloat(radiusMatch[1]);
        }
        
        if (printer) {
            const radiusMatch = printer.content.match(/delta_radius:\s*([\d.]+)/);
            const heightMatch = printer.content.match(/max_z:\s*([\d.]+)/);
            if (radiusMatch) defaults.deltaRadius = parseFloat(radiusMatch[1]);
            if (heightMatch) defaults.deltaHeight = parseFloat(heightMatch[1]);
        }
    } else {
        if (stepperX) {
...
function generate() {
    ...
    const settings = {
        ...
        zLevelingType: document.getElementById('zMotorCount').value === '4' 
            ...
            : document.getElementById('zLevelingType').value,
        ...
        probeOffsetX: parseFloat(document.getElementById('probeOffsetX').value) || 0,
        probeOffsetY: parseFloat(document.getElementById('probeOffsetY').value) || 0,
        sensorlessXY: document.getElementById('sensorlessXY').checked
    };
    
    const enabledSections = new Set();
    document.querySelectorAll('#sections-container input[type="checkbox"]:checked').forEach(cb => {
        enabledSections.add(parseInt(cb.value));
    });
    ...
    // Add header
    output += '# Generated by Klipper Config Generator\n';
    output += `# Based on: ${fileName}\n`;
    output += `# Generated: ${new Date().toISOString()}\n`;
    output += '\n';
    
    // Add includes block
    const includesBlock = generateIncludesBlock();
    if (includesBlock) {
        output += includesBlock + '\n';
    }
    
    // Find SAVE_CONFIG position
    const saveConfigStart = lines.findIndex(line => 
        line.includes('#*# <---------------------- SAVE_CONFIG ---------------------->')
    );
    ...
    for (let i = 0; i < sections.length; i++) {
        ...
        const isEnabled = enabledSections.has(i);
        ...
        sectionLines = modifySectionContent(section.name, sectionLines, settings);
        
        if (isEnabled && wasCommented) {
            // Uncomment the section
            output += sectionLines.map(line => {
                if (line.startsWith('#')) {
                    return line.substring(1).trimStart() ? line.substring(1) : '';
                }
                return line;
            }).join('\n') + '\n\n';
        } else if (!isEnabled && !wasCommented) {
    ...
    // Add SAVE_CONFIG section if present
...
function modifySectionContent(sectionName, lines, settings) {
    const name = sectionName.toLowerCase();
    
    // Modify printer section
    if (name === 'printer') {
        lines = lines.map(line => {
            if (line.match(/^\s*kinematics:/)) {
                return `kinematics: ${settings.kinematicsKlipper}`;
            }
            return line;
        });
    }
    
    // Modify stepper sections for bed size
    if (name === 'stepper_x' && !settings.usesDelta) {
        lines = lines.map(line => {
            if (line.match(/^\s*position_max:/)) {
                return `position_max: ${settings.bedX}`;
            }
            if (line.match(/^\s*position_endstop:/) && settings.endstopX === 'max') {
                return `position_endstop: ${settings.bedX}`;
            }
            return line;
        });
    }
    
    if (name === 'stepper_y' && !settings.usesDelta) {
        lines = lines.map(line => {
            if (line.match(/^\s*position_max:/)) {
                return `position_max: ${settings.bedY}`;
            }
            if (line.match(/^\s*position_endstop:/) && settings.endstopY === 'max') {
                return `position_endstop: ${settings.bedY}`;
            }
            return line;
        });
    }
    
    if (name === 'stepper_z' && !settings.usesDelta) {
        lines = lines.map(line => {
            if (line.match(/^\s*position_max:/)) {
                return `position_max: ${settings.bedZ}`;
            }
            return line;
        });
    }
    ...
    if (name === 'probe' || name === 'bltouch') {
        lines = lines.map(line => {
            if (line.match(/^\s*x_offset:/)) {
                return `x_offset: ${settings.probeOffsetX}`;
            }
            if (line.match(/^\s*y_offset:/)) {
                return `y_offset: ${settings.probeOffsetY}`;
            }
            return line;
        });
    }
...
function generateAdditionalZMotors(settings, sections) {
    ...
    for (let i = 1; i < settings.zMotorCount; i++) {
        ...
        if (settings.zEndstopType === 'probe') {
    ...
    // Add TMC sections for additional Z motors
    const tmcZ = sections.find(s => s.name.toLowerCase().match(/tmc\d+\s+stepper_z$/));
    if (tmcZ) {
        const tmcType = tmcZ.name.split(' ')[0];
        const currentMatch = tmcZ.content.match(/run_current:\s*([\d.]+)/);
    ...
    // Add Z tilt or QGL configuration
...
function resetForm() {
    ...
    if (window.boardOptions && boardSelect) {
    ...
    const kinematicsSelect = document.getElementById('kinematicsSelect');
    if (kinematicsSelect) {
        kinematicsSelect.value = 'cartesian';
        updateKinematicsOptions();
    }
    ...
    const bedZ = document.getElementById('bedZ');
    if (bedX) bedX.value = '235';
    if (bedY) bedY.value = '235';
    if (bedZ) bedZ.value = '250';
    
    // Reset delta dimensions
    ...
    // Reset Z motor count
    const zMotorCount = document.getElementById('zMotorCount');
    if (zMotorCount) {
        zMotorCount.value = '1';
        updateZMotorOptions();
    }
    ...
    // Reset endstops
    const endstopX = document.getElementById('endstopX');
    const endstopY = document.getElementById('endstopY');
    const zEndstopType = document.getElementById('zEndstopType');
    if (endstopX) endstopX.value = 'min';
    if (endstopY) endstopY.value = 'min';
    if (zEndstopType) {
        zEndstopType.value = 'switch_min';
        updateZEndstopOptions();
    }
    ...
    const sensorlessXY = document.getElementById('sensorlessXY');
    if (sensorlessXY) {
        sensorlessXY.checked = false;
        updateSensorlessWarning();
    }
